...
class HTMLTree2Latex:
    ...
    def _to_latex(self, node, inLatex = False):
        ...
        if node['tag'] == 'b':
            ...
            hasLatex = True
            result.append(NoEscape(r'\underline{'))
            endCap.insert(0, NoEscape('}'))
        elif node['tag'] == 'font':
                ...
                for key, val in node['attrs']:
                    if key == 'style':
                        ...
                        if 'background-color' in cssStyle:
                            ...
                            colorItem = self._get_color_item(colorStr)
                            result.append(NoEscape(r'\smash{\colorbox{%s}{\ctmstrut '%colorItem['latex_name']))
                            endCap.insert(0, NoEscape('}}'))
        ...
        for data in node['data']:
            if isinstance(data, str):
                if inLatex:
                    ...
                    result.extend(self._escape_latex_symbols(data))
                else:
